...
METHODS = {
    ...
    "U-Net++": {
        "experiment": "baseline_unetpp",
    },
    "BGD-SF PolySegNet": {
        "experiment": "bgdsf_polysegnet_full",
    },
    "PraNet": {
        "results": "outputs/official_baselines/pranet/results.json",
    },
    "Polyp-PVT": {
        "results": "outputs/official_baselines/polyp_pvt/results.json",
    },
}
...
def _load_prediction_results(path: str) -> Dict[str, Dict[str, float]]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Collect initial baseline results.")
    parser.add_argument(
        "--dataset-roots",
        nargs="+",
        default=[],
    ...
    parser.add_argument("--unet-exp", default=None)
    parser.add_argument("--unetpp-exp", default=None)
    parser.add_argument("--bgdsf-exp", default=None)
    parser.add_argument("--pranet-results", default=None)
    parser.add_argument("--polyp-pvt-results", default=None)
    parser.add_argument(
        "--output",
        default="outputs/tables/initial_baseline_comparison.csv",
    )
    ...
    if args.unet_exp:
        methods["U-Net"]["experiment"] = args.unet_exp
    if args.unetpp_exp:
        methods["U-Net++"]["experiment"] = args.unetpp_exp
    if args.bgdsf_exp:
        methods["BGD-SF PolySegNet"]["experiment"] = args.bgdsf_exp
    if args.pranet_results:
        methods["PraNet"]["results"] = args.pranet_results
    if args.polyp_pvt_results:
        methods["Polyp-PVT"]["results"] = args.polyp_pvt_results
    ...
    for method_name, spec in methods.items():
        if "experiment" in spec:
            ...
            per_dataset = _load_prediction_results(spec["results"])
            params_m = None
